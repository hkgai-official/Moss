...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.MOSS_GATEWAY_TOKEN;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.MOSS_GATEWAY_TOKEN,
    ...
    const token = nextConfig.gateway?.auth?.token ?? process.env.MOSS_GATEWAY_TOKEN;
