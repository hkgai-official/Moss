...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.MOSS_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or MOSS_GATEWAY_TOKEN)",
  );
}
