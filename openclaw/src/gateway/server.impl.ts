...
import {
  ...
  type ServiceHandle as EvolutionServiceHandle,
...
export async function startGatewayServer(
  ...
  // MOSS v0.1: evolution service (heartbeat writer + boot validation +
  // /evolve trigger entry point). Skipped in minimal-test mode (vitest)
  // because it touches /home/node/.openclaw and validates env vars that
  // unit tests don't set. If the inner-repo / env-var validation fails
  // the service still runs heartbeat but `/evolve` rejects with the
  // captured `bootError` — the gateway boot is never blocked.
  let evolutionService: EvolutionServiceHandle | null = null;
  // NOTE: startEvolutionService() here keeps the heartbeat writer alive so
  // the host-daemon supervisor's post-swap probe (checks 0/1/2/3) sees a
  // fresh heartbeat.json after the new image boots. It does NOT itself
  // trigger any evolution loop. Real triggers come from a gateway POST to
  // /api/evolution/trigger with a flag-batch payload (typically invoked
  // via the `moss evo start` CLI).
  if (!minimalTestGateway) {
    try {
      evolutionService = await startEvolutionService();
      if (evolutionService.bootError) {
        log.warn(
          `evolution service started in degraded mode (bootError=${evolutionService.bootError}); /evolve will be unavailable`,
        );
      } else {
        log.info("evolution service started");
      }
    } catch (err) {
      log.error(`failed to start evolution service: ${String(err)}`);
    }
  }
    ...
    close: async (opts) => {
      ...
      if (evolutionService) {
        try {
          ...
          log.warn(`evolution service stop failed: ${String(err)}`);
        }
      }
      await close(opts);
    },
