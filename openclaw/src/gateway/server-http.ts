...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (
        await handleEvolutionHttpRequest(req, res, {
          // In-container: ALWAYS use the mount target. MOSS_DATA_DIR
          // env is the HOST-side path (set in docker-compose so host-daemon
          // can find it) and would EACCES if used inside the gateway.
          // Match the in-container detection pattern used by user-mode.ts.
          dataDir: fs.existsSync("/home/node/.openclaw")
            ? "/home/node/.openclaw"
            : (process.env.MOSS_DATA_DIR ?? "/home/node/.openclaw"),
          openclawRepoDir: process.env.OPENCLAW_REPO_DIR ?? "/app",
          auth: resolvedAuth,
          trustedProxies,
          allowRealIpFallback,
          rateLimiter,
