...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.MOSS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.openclaw === undefined) {
      ...
      process.env.MOSS_GATEWAY_TOKEN = envSnapshot.openclaw;
    }
  ...
  it("uses MOSS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.MOSS_GATEWAY_TOKEN = "local-env-token";
