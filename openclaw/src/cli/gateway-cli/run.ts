...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (opts.token) {
    ...
    if (token) {
      process.env.MOSS_GATEWAY_TOKEN = token;
    }
  }
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or MOSS_GATEWAY_TOKEN/OPENCLAW_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: MOSS_GATEWAY_TOKEN env if set)",
    )
