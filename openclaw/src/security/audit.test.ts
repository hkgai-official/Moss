...
describe("security audit", () => {
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.MOSS_GATEWAY_TOKEN;
    const prevPassword = process.env.OPENCLAW_GATEWAY_PASSWORD;
    delete process.env.MOSS_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.MOSS_GATEWAY_TOKEN = prevToken;
      }
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.MOSS_GATEWAY_TOKEN;
    process.env.MOSS_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: OpenClawConfig = {
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    beforeEach(() => {
      envSnapshot = captureEnv(["MOSS_GATEWAY_TOKEN", "OPENCLAW_GATEWAY_PASSWORD"]);
      delete process.env.MOSS_GATEWAY_TOKEN;
      delete process.env.OPENCLAW_GATEWAY_PASSWORD;
    });
    ...
    const setProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        process.env.MOSS_GATEWAY_TOKEN = env.token;
      }
