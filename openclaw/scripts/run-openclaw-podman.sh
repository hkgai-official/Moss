...
MOSS_IMAGE="${OPENCLAW_PODMAN_IMAGE:-openclaw:local}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate MOSS_GATEWAY_TOKEN." >&2
  exit 1
}

if [[ -z "${MOSS_GATEWAY_TOKEN:-}" ]]; then
  export MOSS_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "MOSS_GATEWAY_TOKEN" "$MOSS_GATEWAY_TOKEN"
  echo "Generated MOSS_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -e MOSS_GATEWAY_TOKEN="$MOSS_GATEWAY_TOKEN" \
