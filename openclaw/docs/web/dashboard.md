...
- If the UI prompts for auth, paste the token from `gateway.auth.token` (or `MOSS_GATEWAY_TOKEN`) into Control UI settings.

## Token basics (local vs remote)

- **Localhost**: open `http://127.0.0.1:18789/`.
- **Token source**: `gateway.auth.token` (or `MOSS_GATEWAY_TOKEN`); the UI stores a copy in localStorage after you connect.
