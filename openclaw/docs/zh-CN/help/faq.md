...
- 令牌与 `gateway.auth.token`（或 `MOSS_GATEWAY_TOKEN`）的值相同，UI 在首次加载后会存储它。
...
非 local loopback 绑定**需要认证**。配置 `gateway.auth.mode` + `gateway.auth.token`（或使用 `MOSS_GATEWAY_TOKEN`）。
...
- 在 Gateway 网关主机上设置 `gateway.auth.token`（或 `MOSS_GATEWAY_TOKEN`）。
