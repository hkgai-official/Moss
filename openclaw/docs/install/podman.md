...
- **Token:** Stored in `~openclaw/.openclaw/.env` as `MOSS_GATEWAY_TOKEN`. `setup-podman.sh` and `run-openclaw-podman.sh` generate it if missing (uses `openssl`, `python3`, or `od`).
