...
fly secrets set MOSS_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `MOSS_GATEWAY_TOKEN` for security.
...
Paste your gateway token (the one from `MOSS_GATEWAY_TOKEN`) to authenticate.
