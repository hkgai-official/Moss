...
- If it asks for auth, paste the token from `gateway.auth.token` (or `MOSS_GATEWAY_TOKEN`) into Control UI settings.
...
Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `MOSS_GATEWAY_TOKEN`).
...
- Set `gateway.auth.token` (or `MOSS_GATEWAY_TOKEN`) on the gateway host.
