...
IMAGE_NAME="${MOSS_IMAGE:-openclaw:local}"
...
if [[ -z "${MOSS_GATEWAY_TOKEN:-}" ]]; then
  if command -v openssl >/dev/null 2>&1; then
    MOSS_GATEWAY_TOKEN="$(openssl rand -hex 32)"
  else
    MOSS_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
echo "  - Gateway token: $MOSS_GATEWAY_TOKEN"
...
echo "Token: $MOSS_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f openclaw-gateway"
echo "  ${COMPOSE_HINT} exec openclaw-gateway node dist/index.js health --token \"$MOSS_GATEWAY_TOKEN\""
